...
In order to manually visit them first. I'll just set up a custom binding that adds all files under my code directories into the project cache.

It took too long on startup to refresh projects in known source directory roots. It locks the emacs loop, emacs is not multi-threaded, so trying to get this to work in the background isn't viable.
